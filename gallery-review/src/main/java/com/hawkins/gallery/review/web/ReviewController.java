...
public class ReviewController {
    ...
    public String page(@RequestParam(defaultValue="Flagged") String filter,
                       @RequestParam(defaultValue="0.65") double threshold,
    ...
    public Stats stats(@RequestParam(defaultValue="0.65") double threshold) {
        return new Stats(review.counts(threshold), queue.stats());
    }

    @PostMapping("/status") @ResponseBody
    public ResponseEntity<?> status(@RequestBody Selection request) {
        return ResponseEntity.ok(java.util.Map.of("changed", review.setStatus(request.ids(), ReviewStatus.valueOf(request.status()))));
    }

    @PostMapping("/queue") @ResponseBody
    public ResponseEntity<?> queue(@RequestBody QueueRequest request) {
    ...
    public ResponseEntity<?> quarantine(@RequestBody Ids request) throws Exception {
        return ResponseEntity.ok(java.util.Map.of("changed", review.quarantine(request.ids())));
    }

    @PostMapping("/restore") @ResponseBody
    public ResponseEntity<?> restore(@RequestBody Ids request) throws Exception {
    ...
    public record Selection(List<String> ids, String status) { }
    public record QueueRequest(List<String> ids, boolean force) { }
